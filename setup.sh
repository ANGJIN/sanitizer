...
print_yellow "\nPull latest sanitizer image"
docker pull zmfjrxmfjr/sanitizer:latest
if [ $? -eq 0 ]
then
...
docker run --rm --name sanitizer -v "${inputfile}:/opt/src" -v "${outputfile}:/opt/results" zmfjrxmfjr/sanitizer:latest planetql setup
if [ $? -eq 0 ]
then
